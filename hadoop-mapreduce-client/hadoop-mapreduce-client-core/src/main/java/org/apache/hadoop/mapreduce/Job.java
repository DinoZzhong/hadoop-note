...
public class Job extends JobContextImpl implements JobContext {  
  ...
  private synchronized void connect()  // 根据参数mapreduce.framework.name初始化集群 本质是协议信息配置 ClientProtocol
  ...
  public void submit() 
         throws IOException, InterruptedException, ClassNotFoundException {
    ensureState(JobState.DEFINE);
    setUseNewAPI(); //设置使用新的api
    connect(); // 初始化cluster,即获取到集群信息，包括是采用yarn还是local方式
    final JobSubmitter submitter =  // 构建一个提交器
  ...
  public boolean waitForCompletion(boolean verbose
                                            ...
                                            ClassNotFoundException {
    if (state == JobState.DEFINE) { //mapreduce main方法入口

      submit(); // 提交任务执行(包括做一些初始化)
    }
    if (verbose) { //监控和打印
      monitorAndPrintJob();
    } else {
