...
class JobSubmitter {
  ...
  private ClientProtocol submitClient;  //客户端通信协议 用于集群交互完成作业提交和作业状态查询
  ...
  throws ClassNotFoundException, InterruptedException, IOException {
    ...
    checkSpecs(job); // 作业输出路径获取和检查

    Configuration conf = job.getConfiguration();
    addMRFrameworkToDistributedCache(conf); //添加应用jar到分布式缓存中

    Path jobStagingArea = JobSubmissionFiles.getStagingDir(cluster, conf);
    //configure the command line options correctly on the submitting dfs
    InetAddress ip = InetAddress.getLocalHost();
    if (ip != null) { // 设置提交作业的主机信息到配置中
    ...
    try {
      ...
      LOG.debug("Configuring job " + jobId + " with " + submitJobDir 
          + " as the submit dir");
      // get delegation token for the dir  路径文件的授权令牌获取
      ...
      // map 数量获取，即获取分片数据文件和分片元数据文件spilitmetainfo,并取得map数量
      ...
      // to job file.
      // 获取作业队列名,取参数mapredcue.job.queuename,默认是default
      ...
      // different job.
      TokenCache.cleanUpTokenReferral(conf); // 清空缓存令牌
      ...
      /**通过构造的客户端通信协议实例提交job并获取作业状态
       * {@link YARNRunner,LocalJobRunner} 根据协议构建
       * */
