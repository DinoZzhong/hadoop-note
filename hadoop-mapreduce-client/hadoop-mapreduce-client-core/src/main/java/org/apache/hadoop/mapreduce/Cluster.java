...
public class Cluster {
  ...
  private Configuration conf; // 配置下细腻
 ...
 * */
  ...
  public Cluster(InetSocketAddress jobTrackAddr, Configuration conf)  // 初始化集群信息
      throws IOException {
    this.conf = conf;
  ...
  private void initialize(InetSocketAddress jobTrackAddr, Configuration conf)
    ...
    synchronized (frameworkLoader) { // 由于有两个类，会先去尝试实现localRunner 如果为空，然后在处理yarnRunner
      for (ClientProtocolProvider provider : frameworkLoader) {
        LOG.debug("Trying ClientProtocolProvider : "
            + provider.getClass().getName());
        ClientProtocol clientProtocol = null; 
        try {
          // 在create里配置是yarn还是local模式的runner
